...
const fetchOneProduct = async () => {
  try {
    const res = await fetch(`https://realauto.limsa.uz/api/cars/${route.params.id}`);
    const data = await res.json();
...
const bground ="border:1px solid #7C7C7C;"



// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~input telegram submit~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
const username = ref("");
const telephone = ref("");
const period = ref("")
const detailes = ref("")
...
function submit(){
    const fullMessage= `Car: ${model.value}, Color: ${color.value}, Name: ${username.value}, telephone: ${telephone.value}, Period : ${period.value}, Detalies: ${detailes.value} `;
...
<template>
  <div class="container py-10">
    <h1 class="text-white text-[28px] text-center p768:text-left p768:ml-6 p768:text-[50px] font-semibold uppercase">{{Oneproduct?.model?.name}} ({{Oneproduct?.color}})</h1>
<div class="idProducts p1200:flex justify-between">
  <!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~LEFT~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
  <div class="idProducts__left p-2">
    <!-- ~~~~~~~~~~~~~~~~swipers~~~~~~~~~~~~~~~~~ -->
    <div class="swiperCon max-w-[600px] mx-auto">

      <Carousel id="gallery" loop :items-to-show="1" :wrap-around="false" v-model="datanumb">

    <Slide v-for="(item) in Oneproduct.car_images" :key="item">
      <div class="w-full p1200:w-[550px]  h-[250px] p480:h-[350px] p768:h-[500px] flex justify-center items-center">
        <img class="w-full h-full" :src="'https://realauto.limsa.uz/api/uploads/images/' + item.image.src" alt="">
      </div>
    </Slide>
  ...
  <Carousel
    ...
    ref="carousel"
  >
    <Slide v-for="(item, index) in Oneproduct.car_images" :key="item">
      <div class="w-full border h-[150px] m-[5px] flex justify-center items-center" @click="slideTo(index - 1)">
        <img class="w-full h-full" :src="'https://realauto.limsa.uz/api/uploads/images/' + item.image.src" alt="">
      </div>
    </Slide>
  </Carousel>
...
<!-- ~~~~~~~~~~~~~~~~~~~~~telegram browsers~~~~~~~~~~~~~~ -->
<div class="tg-con flex gap-3 w-full justify-center my-[30px]">
  <button class="w-[75px] h-[65px] bg-[#5AEC5A] text-white text-[35px] rounded-2xl  flex justify-center items-center">
    <a href="">
...
<button class="w-[75px] h-[65px] bg-[#2374F8] text-white text-[35px] rounded-2xl  flex justify-center items-center">
...
<button class="w-[75px] h-[65px] bg-[#FFB630] text-white text-[35px] rounded-2xl  flex justify-center items-center">
...
<form action="" method="POST" @submit.prevent="submit"
 class="border p-[30px] flex flex-col gap-5 max-w-[400px] w-full mx-auto">
  <h1 class="text-white">{{ Oneproduct?.model?.name }} / {{Oneproduct?.color }}</h1>
 <input required v-model="username" class="w-full px-3 py-5" :placeholder="$t('Name')" type="text">
 <input required v-model="telephone" class="w-full px-3 py-5" :placeholder="$t('Phone')" type="number">
 <input required v-model="period" class="w-full px-3 py-5" :placeholder="$t('Period')" type="number">
 <input required v-model="detailes" class="w-full px-3 py-5" :placeholder="$t('Detailes')" type="text">
 <button @click="modelinput(Oneproduct?.model?.name),colorinput(Oneproduct?.color)" type="submit" class="btn__send w-[150px] bg-orange-600  p-3 font-semibold text-[20px] rounded-[2px]">{{ $t('Send') }}</button>
</form>
<!-- ~~~~~~~~~~~~~~~~~~~~~FORM dan kegin mini text~~~~~~~~~~~~~~ -->
<article class="text text-[#a4a4a4] mt-8 mx-[20px] text-center p1200:text-left p768:mx-0 p768:text-[12px] flex flex-col gap-1 p768:mt-[80px]">
 <p>{{ $t("The price doesn't include additional 5% VAT.") }}</p>
 <p>{{ $t('There is a 3% transaction fee when paying by credit/debit card.') }}</p>
 <p>{{ $t('There is a 7% transaction fee when paying with American Express.') }}</p>
</article>
 ...
 <h3 class="text-white text-[32px] font-semibold mt-[50px] mb-[25px] mx-[10px] text-center p1200:text-left">{{ $t('SIMILAR OFFERS') }}</h3>
 <div class="allproductsCon flex flex-wrap gap-2 justify-center p1200:justify-start p-1">
  <RouterLink @click="handlechange(),scrollTop()"  :to="'/product/' + item?.id" v-for="item in product" :key="item">
<card :car="item" :bg="bground"/>
</RouterLink>
 </div>
